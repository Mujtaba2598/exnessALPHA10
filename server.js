...
async function tickerallRequest(endpoint, method = 'GET', data = null) {
    const endpoints = [
        ...
        endpoint.replace('/v1/', '/'),
        endpoint.replace('/v1', ''),
        endpoint.replace('/sessions/start', '/session/start'),
        endpoint.replace('/accounts/', '/account/'),
        endpoint.replace('/market/price/', '/market/price?symbol='),
        endpoint.replace('/orders/place/', '/orders/place?accountId='),
        endpoint.replace('/orders/close/', '/orders/close?orderId='),
    ];
    
    // Try different base URLs
    const baseUrls = [
        'https://api.tickerall.com/v1',
        'https://api.tickerall.com',
        'https://tickerall.com/api/v1',
        'https://tickerall.com/api'
    ];
        ...
        for (const ep of endpoints) {
            try {
                ...
                if (error.response && error.response.status !== 404) {
                    // If it's not a 404, it might be a real error, but we'll continue trying
                }
            }
        }
    }
    
    console.error('All TickerAll API attempts failed');
    throw lastError || new Error('TickerAll API unavailable');
}
...
app.post('/api/set-exness-creds', authenticate, async (req, res) => {
    try {
        ...
        // Try to start session with TickerAll
        let session;
        try {
            session = await tickerallRequest('/sessions/start', 'POST', {
                broker: 'mt5',
                server: exnessServer,
            ...
            session = await tickerallRequest('/connect', 'POST', {
                broker: 'mt5',
                server: exnessServer,
        ...
        const accountId = session.accountId || session.id || session.account_id || session.sessionId;
        console.log(`✅ Session created: ${accountId}`);
        
        // Get account info
        let accountInfo;
        try {
            accountInfo = await tickerallRequest(`/accounts/${accountId}`, 'GET');
        } catch (error) {
            accountInfo = await tickerallRequest(`/account/${accountId}`, 'GET');
        }
        ...
        res.status(401).json({ 
            success: false, 
            message: 'Connection failed. Please check your Exness credentials and try again.' 
        });
    }
});
...
app.post('/api/connect-exness', authenticate, async (req, res) => {
    try {
        ...
        let accountInfo;
        try {
            accountInfo = await tickerallRequest(`/accounts/${user.tickerallSessionId}`, 'GET');
        } catch (error) {
            accountInfo = await tickerallRequest(`/account/${user.tickerallSessionId}`, 'GET');
        }
...
async function getAISignal(symbol, accountId) {
    try {
        let rates;
        try {
            rates = await tickerallRequest(`/market/history/${accountId}`, 'POST', {
                symbol: symbol,
                timeframe: 'M5',
                limit: 100
            });
        } catch (error) {
            rates = await tickerallRequest(`/market/history`, 'POST', {
                accountId: accountId,
                symbol: symbol,
                timeframe: 'M5',
                limit: 100
        ...
        let action = 'BUY';
        let confidence = 0.6;
        let reasons = ['Default buy signal'];
        
        if (rsi < 30) {
            action = 'BUY';
            confidence = 0.85;
            reasons = [`RSI oversold (${rsi.toFixed(1)})`];
        } else if (rsi < 40 && ma20 > ma50) {
            action = 'BUY';
            confidence = 0.75;
            reasons = [`RSI ${rsi.toFixed(1)} in buy zone, uptrend`];
        } else if (momentum > 0.2 && ma20 > ma50) {
            action = 'BUY';
            confidence = 0.7;
            reasons = [`Positive momentum ${momentum.toFixed(2)}%`];
        } else if (rsi > 70) {
            action = 'SELL';
            confidence = 0.85;
            ...
            action = 'SELL';
            confidence = 0.75;
            reasons = [`RSI ${rsi.toFixed(1)} in sell zone, downtrend`];
        } else if (momentum < -0.2 && ma20 < ma50) {
            action = 'SELL';
            confidence = 0.7;
            reasons = [`Negative momentum ${momentum.toFixed(2)}%`];
        } else {
            // Default to buy with moderate confidence
            action = Math.random() > 0.5 ? 'BUY' : 'SELL';
            confidence = 0.55;
            reasons = [`Neutral market, ${action.toLowerCase()} signal`];
        }
        ...
        return { action: 'BUY', confidence: 0.6, reasons: ['Error, attempting buy'], currentPrice: 0 };
    }
}
...
async function shouldClosePosition(position, accountId) {
    try {
        let price;
        try {
            price = await tickerallRequest(`/market/price/${accountId}/${position.symbol}`, 'GET');
        } catch (error) {
            price = await tickerallRequest(`/market/price`, 'GET', {
                accountId: accountId,
        ...
        if (profitPercent > 0) {
            if (profitPercent >= 1.5) {
                shouldClose = true;
                reason = `Profit ${profitPercent.toFixed(2)}%`;
            } else if (profitPercent >= 0.5) {
                if (Math.random() > 0.7) {
                    shouldClose = true;
                    reason = `Taking ${profitPercent.toFixed(2)}% profit`;
                }
            }
...
class HalalTradingEngine {
    ...
    async executeTrade(symbol, side, signal) {
        ...
        try {
            let accountInfo;
            try {
                accountInfo = await tickerallRequest(`/accounts/${this.accountId}`, 'GET');
            } catch (error) {
                accountInfo = await tickerallRequest(`/account/${this.accountId}`, 'GET');
            }
            ...
            let price;
            try {
                price = await tickerallRequest(`/market/price/${this.accountId}/${symbol}`, 'GET');
            } catch (error) {
                price = await tickerallRequest(`/market/price`, 'GET', {
            ...
            console.log(`📈 Opening ${side.toUpperCase()} for ${symbol}`);
            ...
            try {
                ...
                order = await tickerallRequest(`/orders/place`, 'POST', {
                    accountId: this.accountId,
                    type: 'market',
                    symbol: symbol,
                    side: side.toUpperCase(),
            ...
            console.log(`✅ ${side.toUpperCase()} opened`);
        } catch (error) {
    ...
    async closePosition(position, profitPercent, currentPrice) {
        try {
            try {
                ...
                await tickerallRequest(`/orders/close`, 'POST', {
                    accountId: this.accountId,
            ...
            console.log(`✅ CLOSED ${position.symbol} | Profit: $${profit.toFixed(2)} (${profitPercent.toFixed(2)}%)`);
        } catch (error) {
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        ...
        res.json({ 
            success: true, 
            sessionId, 
            message: `✅ HALAL TRADING STARTED! Investment: $${investmentAmount} | Target: $${targetProfit}` 
        });
    } catch (error) {
